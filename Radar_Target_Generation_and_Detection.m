...
position_ini = 100; % 200m
Velocity = 30; %[-70,-30,0,30,70];
...
%calculated_range = c*T_chirp * fb/(2*B_sweep);
...
%normalize.
signal_fft = fft(X(1:Nr));

 % *%TODO* :
% Take the absolute value of FFT output
%signal_fft = abs(signal_fft/length(signal_fft));
...
% CFAR
   ...
   for i = Tr+Gr+1:(Nr-(Tr + Gr))
       for j = Td+Gd+1:(Nd-(Gd+Td))
           ...
           threshold(i,j) = pow2db(noise_level(i,j)/numbercum);%pow2db(noise_level(i,j)/(2*(Td+Gd+1)*2*(Tr+Gr+1)-(Gr*Gd)-1));
...
[X_plot,Y_plot] = meshgrid(doppler_axis,range_axis);
Z_plot = RDM;
figure(4)
